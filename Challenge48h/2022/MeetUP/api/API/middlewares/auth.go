...
func JWT(c *gin.Context) {
	...
	if len(repository.GetUserByEmail(claims.Email)) == 0 && len(repository.GetAdminByEmail(claims.Email)) == 0 && len(repository.GetOrgaByEmail(claims.Email)) == 0 {
...
func GetJWT(c *gin.Context) {
	...
	c.Writer.Header().Add("Autorization", "Bearer " + token)
