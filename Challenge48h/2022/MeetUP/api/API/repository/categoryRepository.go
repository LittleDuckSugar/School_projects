...
func GetCategoryById(id string) models.Category {
	rows, err := currentDB.Query("SELECT * FROM category WHERE category_id = $1", id)
	...
	return models.Category{Id: uuid, Name: name}
}
...
func GetCategories() []models.Category {
	...
	for rows.Next() {
		...
		categories = append(categories, models.Category{Id: uuid, Name: name})
	}
...
func PutCategoryById(uuid string, updatedCategory models.PostCategory) {
	// dynamic
	updateDynStmt := `update "category" SET name = $2 where category_id = $1`
...
func DeleteCategoryById(uuid string) {
	// dynamic
	deleteDynStmt := `delete from "category" where category_id = $1`
